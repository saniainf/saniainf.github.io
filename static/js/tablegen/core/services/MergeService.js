// MergeService.js
// Операции объединения (merge) и разделения (split). Объединение превращает прямоугольный диапазон в одну ведущую ячейку.
// Остальные ячейки диапазона удаляются из массива (их покрывает rowSpan/colSpan ведущей).
...
export function mergeRange(model, r1, c1, r2, c2) {
  ...
  // Сбор содержимого всех ячеек внутри диапазона ДО модификации массива: склеиваем значения сверху вниз, слева направо.
  const collected = [];
...
/**
 * Разъединить (split) все объединённые ячейки, которые попадают (частично или полностью) в диапазон.
 * Алгоритм: фиксируем координаты ведущих ячеек, пересекающих прямоугольник, затем вызываем splitCell по списку.
 * Одна большая область, накрывающая диапазон целиком, разъединяется однократно.
 * @param {import('../model/TableModel.js').TableModel} model
 * @param {number} r1
 * @param {number} c1
 * @param {number} r2
 ...
 * Поведение по умолчанию (overlap) соответствует ожиданиям: все пересекающиеся области распадаются.
 */
